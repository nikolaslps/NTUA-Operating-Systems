...
void increase_workers(int value){
    ...
    if (length > 0 && length < sizeof(message)) {
        ...
        exit(1);
         ...
         int length = snprintf(message, sizeof(message),"Couldn't increase because I cant have so many >    if (length > 0 && length < sizeof(message)) {
        sendMessageWithWrite(message);
    }
    else {
        perror("snprintf");
...
void decrease_workers(int value){
    ...
    if (length > 0 && length < sizeof(message)) {
        ...
        int length = snprintf(message, sizeof(message),"Couldn't decrease because no one will be left o>    if (length > 0 && length < sizeof(message)) {
        sendMessageWithWrite(message);
    }
    else {
        perror("snprintf");
...
void get_info(){
    ...
    char message[256];
        int length = snprintf(message, sizeof(message), "All the available workers who have already com>    if (length > 0 && length < sizeof(message)) {
        sendMessageWithWrite(message);
    }
    else {
        perror("snprintf");
...
void give_file_percentage_and_count_till_now(){
    ...
    char message[100];
    int message_length = snprintf(message, sizeof(message), "the percentage of the file examined up til>    if (message_length < 0 || message_length >= sizeof(message)) {
        perror("snprintf");
        exit(1);
    }
...
{ // This will check if a process was terminated while searching and has left it's share of work still >    for (int j = 0; j <= current_worker; j++)
    {
        if (WORKERS[j].active == -2) // I only check start as end will always have the same values as s>            return j;
    }
    return -1;
}
...
void handle_sigchild(int sig) // This signal informs the dispatcher that a child finished it's work cor>{
    int status;
    pid_t pid;
    int counter, i;
                ...
                exit(1);
                    ...
                    execv("./worker", argv2);
                     exit(1);
                        ...
                        char *argv2[] = {"./worker", argv[1], &cc, start_str, end_str, start_str2, end_>                        execv("./worker", argv2);
                     exit(1);
                    }
