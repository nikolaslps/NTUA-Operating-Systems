...
//Global semaphores and number of processes
sem_t *semaphores;
int prcnt;
int process_number;
...
void compute_and_output_mandel_line()
{
	...
	for (int i = process_number; i < y_chars; i += prcnt){
		...
		if (sem_wait(&semaphores[process_number]) != 0){
			perror("sem_wait");
        	        exit(1);
		}

		output_mandel_line(1, color_val);
		if (sem_post(&semaphores[(process_number+1) % prcnt]) != 0){
                	perror("sem_signal");
			exit(1);
        	}
	}

	destroy_shared_memory_area(semaphores, prcnt * sizeof(sem_t));
	exit(0);
}
...
int main(int argc, char **argv)
{
	...
	semaphores = create_shared_memory_area(prcnt * sizeof(sem_t));
