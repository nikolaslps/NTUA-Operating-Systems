...
void child(void)
{
	 ...
	 */
	printf("Physical address of child heap_private_buf: %ld\n", get_physical_address((uint64_t)heap_private_buf));
	 ...
	 */
        ...
        heap_private_buf[i] = 42;

	printf("Physical address of child heap_private_buf after writing: %ld\n", get_physical_address((uint64_t)heap_private_buf));
	...
	printf("Physical address of child heap_shared_buf: %ld\n", get_physical_address((uint64_t)heap_shared_buf));
	...
	/* Change the privilages for the child process to read_only */
	if(mprotect(heap_shared_buf, buffer_size, PROT_READ) == -1)
	{
		perror("mprotect");
		exit(1);
	}
	printf("Memory map of child precess after changing the privilages to read-only:\n");
	show_maps(); 
...
void parent(pid_t child_pid)
{
	...
	printf("Child process map from parent's function:\n");
	show_maps();
	 ...
	 */
	printf("Physical address of parent heap_private_buf: %ld\n", get_physical_address((uint64_t)heap_private_buf));
	 ...
	 */
	printf("Physical address of parent heap_private_buf after writing: %ld\n", get_physical_address((uint64_t)heap_private_buf));
	 ...
	 */
	printf("Physical address of parent heap_shared_buf: %ld\n", get_physical_address((uint64_t)heap_shared_buf));
	 ...
	 */
	printf("Memory map of parent process after changing the privilages of its child to read-only:\n");
	show_maps();
	show_va_info((uint64_t)heap_shared_buf);
...
int main(void)
{
	...
	show_maps();//virtual memory map of process
	/*
	 ...
	 */
	//Filling yp the buffer with zeros
	for (int i = 0; i < buffer_size; i++)
